...
Produces 6 files in the repo root:
    docker-compose-small.yml   / nginx-small.conf    (1 shard,  ~7 CPUs)
    docker-compose-medium.yml  / nginx-medium.conf   (5 shards, ~50 CPUs)
    docker-compose-large.yml   / nginx-large.conf    (10 shards, ~91 CPUs)
"""
...
def generate_compose(size_name: str, cfg: dict) -> str:
    ...
    w("x-orchestrator-base: &orchestrator-base")
    w("  image: orchestrator:latest")
    w("  restart: always")
    w("  env_file: [ env/saga_redis.env, env/checkout.env ]")
    w("  deploy:")
    w("    resources:")
    w("      limits:")
    w(f"        cpus: \"{oc}\"")
    ...
    # -- Orchestrator shards --
    w(f"  # --- Orchestrator shards ---")
    w("")
    for i in range(shards):
        w(f"  orchestrator-service-{i}:")
        w(f"    <<: *orchestrator-base")
        if i == 0:
            w(f"    build: {{ context: ., dockerfile: orchestrator/Dockerfile }}")
        w(f"    environment:")
        w(f"      SHARD_ID: \"{i}\"")
        w(f"      SHARD_COUNT: \"{shards}\"")
        w(f"      REDIS_HOST: orchestrator-db-{i}")
        w(f"      REDIS_PORT: \"6379\"")
        w(f"      REDIS_PASSWORD: redis")
        w(f"      REDIS_DB: \"0\"")
        w(f"    depends_on: [orchestrator-db-{i}, {saga_deps}]")
        w("")

    for i in range(shards):
        w(f"  orchestrator-db-{i}:")
        w(f"    <<: *redis-base")
    w("")
...
def main():
    ...
    for size_name, cfg in SIZES.items():
        ...
        total = (
            ...
            + shards * float(cfg["orchestrator_cpus"])
            + shards * 4 * float(cfg["redis_cpus"])
            + shards * float(cfg["saga_cpus"])
            + float(cfg["gw_cpus"])
        )
        containers = shards * 9 + 1
